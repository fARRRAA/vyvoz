...
class _RouteFinishPageState extends State<RouteFinishPage> {
  ...
  void _loadOrders() {
    setState(() {
      _orders = Api.attachedOrders
          .where((order) => order.orderStatusId == 2 || order.orderStatusId == 3)
          .toList();
    });
  }
  ...
  Widget _buildOrdersInfo() {
    ...
    final totalVolume = _orders.fold(0, (sum, order) => sum + order.wasteVolume);
  ...
  void _openRoute() async {
    if (_currentPosition == null || _plant?.latitude == null || _plant?.longitude == null) {
    ...
    final url = 'https://yandex.ru/maps/?mode=routes&rtext=${_currentPosition!.latitude},${_currentPosition!.longitude}~${_plant!.latitude},${_plant!.longitude}';
